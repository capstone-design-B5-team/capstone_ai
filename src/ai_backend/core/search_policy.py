...
def source_quality_reasons(url: str) -> list[str]:
    """Return source-type labels for a URL."""
    domain = _domain(url)
    if not domain:
        return []

    reasons: list[str] = []
    if _domain_matches(domain, (*_KR_OFFICIAL_DOMAINS, *_GLOBAL_OFFICIAL_DOMAINS)):
...
def source_domain(url: str) -> str:
    """Return a normalized netloc for display/debug payloads."""
    return _domain(url)
