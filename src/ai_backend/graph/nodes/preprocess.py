...
_AVERITEC_TYPE_MAP: dict[str, list[ClaimType]] = {
    "Event/Property Claim": ["FACT", "SOURCE"],  # SOURCE 추가: provenance 앵글 증거 보강(실험 검증, ev 0.138→0.250)
    "Causal Claim":         ["FACT"],
...
def preprocess_node(
    ...
    *,
    ...
    # averitec_claim_types가 제공된 경우: raw_text 자체가 검증 대상 claim이므로 LLM 추출 건너뜀
    if state.get("averitec_claim_types"):
